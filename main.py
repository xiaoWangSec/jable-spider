...
if __name__ == '__main__':
    ...
    for i in tqdm(range(1, 5), colour='green', desc='进度: '):
        time.sleep(1)
        urls = [f"https://fs1.app/new-release/?mode=async&function=get_block&block_id=list_videos_common_videos_list&sort_by=release_year&from={str(i)}", f"https://fs1.app/latest-updates/?mode=async&function=get_block&block_id=list_videos_latest_videos_list&sort_by=post_date&from={str(i)}"]
        for item in urls:
